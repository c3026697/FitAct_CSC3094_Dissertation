...
def execute(workout_id):
    ...
    return render_template(
        "tracking/execute.html", workout=workout, exercises=exercises
    )
...
def log_workout(workout_id):
    ...
    for we in exercises:
        ...
        if set_num == 1:
            sets_done = int(
                request.form.get(f"sets_{we.id}", we.sets_target) or we.sets_target
            )
            reps_done = int(
                request.form.get(f"reps_{we.id}", we.reps_target) or we.reps_target
            )
            db.session.add(
                LoggedExercise(
                    log_id=log.id,
