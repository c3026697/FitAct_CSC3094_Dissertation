...
from app.models import ProgrammeWorkout, Workout, WorkoutExercise, Exercise, SavedWorkout
...
def workout_page():
    ...
    return render_template('workouts/workout_page.html',
                           workout=workout, programme=programme, exercises=exercises)
...
def change_workout():
    ...
    return render_template('workouts/change_workout.html',
                           programme=programme, programme_workouts=programme_workouts)
...
def save_workout(workout_id):
    existing = SavedWorkout.query.filter_by(user_id=current_user.id, workout_id=workout_id).first()
    if not existing:
...
def delete_saved(saved_id):
    sw = SavedWorkout.query.filter_by(id=saved_id, user_id=current_user.id).first_or_404()
...
def create_custom():
    ...
    if request.method == 'POST':
        ...
        workout = Workout(name=name, type='Custom', is_custom=True,
